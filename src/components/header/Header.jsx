...
const Header = () => {
  return (
    <header className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-dark">
      ...
      <div className="relative z-10 flex flex-col items-start text-left px-4 max-w-7xl w-full mx-auto sm:px-6 lg:px-8">
        <h1 className="text-5xl sm:text-7xl md:text-8xl font-heading font-bold mb-6 tracking-tight animate-fade-in-up text-left">
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-gray-200 to-gray-400">
            {headerData.name}
          </span>
        </h1>

        <div className="h-1 w-24 bg-gradient-to-r from-primary to-secondary rounded-full mb-8 animate-zoom-in ml-0"></div>

        <p className="text-xl sm:text-2xl md:text-3xl font-light text-gray-300 mb-4 animate-fade-in-up text-left" style={{ animationDelay: '0.2s' }}>
          {headerData.title}
        </p>

        <p className="text-base sm:text-lg text-gray-400 max-w-2xl mr-auto leading-relaxed animate-fade-in-up text-left" style={{ animationDelay: '0.4s' }}>
          {headerData.description}
        </p>

        <div className="mt-12 animate-fade-in-up text-left" style={{ animationDelay: '0.6s' }}>
